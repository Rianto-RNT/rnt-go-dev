...
func ParseTime(input string) (Time, error) {
	...
	if len(components) != 3 {
		...
		if minute > 59 || minute < 0 {
			return Time{0,0,0}, &TimeParseError{"Hour out of range: 0 <= minute <= 59", fmt.Sprintf("%v", minute)}
		}
		if second > 59 || second < 0 {
			return Time{0,0,0}, &TimeParseError{"Hour out of range: 0 <= hour <= 59", fmt.Sprintf("%v", second)}
		}
